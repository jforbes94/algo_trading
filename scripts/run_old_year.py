"""Run the full pipeline on the older year of data only (backfilled period).

Produces:
  trade_log_old_year.csv
  trade_report_old_year.pdf

Compare against trade_report.pdf (recent year) to test timing luck.
"""
import sys
import os
...
from backtest.backtester import run, trade_log, trade_summary, print_summary
from data.store import load_etf_universe
from data.universe import get_sector_map

# ── Date filter: keep only the older year ─────────────────────────────────────
# Today is ~Jun 2026; 1 year ago = ~Jun 2025; 2 years ago = ~Jun 2024
CUTOFF = datetime.now() - timedelta(days=365)   # everything BEFORE this = old year
...
print("Building features on full 2yr dataset...")
features_df = build_features(data_dict, etf_dict=etf_dict, sector_map=sector_map)

# Filter to old year only
dates = features_df.index.get_level_values("datetime")
old_mask = dates < CUTOFF
features_old = features_df.loc[old_mask].copy()

date_min = features_old.index.get_level_values("datetime").min()
date_max = features_old.index.get_level_values("datetime").max()
print(f"Old year window: {date_min.date()} → {date_max.date()}  ({len(features_old):,} rows)")

# ── Model ─────────────────────────────────────────────────────────────────────
print("\nRunning walk-forward model on old year...")
model   = WalkForwardModel(n_splits=4)
proba   = model.fit_predict(features_old)

# ── Signals & backtest ────────────────────────────────────────────────────────
print("Generating signals...")
signals  = generate_signals(proba, features_old, top_n=20, bottom_n=20)
results  = run(signals, holding_period=4, cost_bps=5.0)

print("\n=== Old Year Backtest Summary ===")
print_summary(results)

log     = trade_log(signals, holding_period=4, cost_bps=5.0, sector_map=sector_map)
summary = trade_summary(log)

log_path = os.path.join(ROOT, "trade_log_old_year.csv")
log.to_csv(log_path, index=False)
print(f"\nTrade log saved: {log_path}  ({len(log):,} trades)")
...
out_path = os.path.join(ROOT, "trade_report_old_year.pdf")
...
with PdfPages(out_path) as pdf:
    ...
    fig.text(0.5, 0.96, "Backtest Trade Report — OLD YEAR", ha="center", va="top",
    ...
    ax_eq.fill_between(eq.index, 1, eq.values, where=eq.values >= 1, alpha=0.15, color=GREEN)
    ax_eq.fill_between(eq.index, 1, eq.values, where=eq.values < 1,  alpha=0.15, color=RED)
    ...
    monthly_df = pd.DataFrame({"ret": monthly.values, "month": monthly.index.month, "year": monthly.index.year})
    ...
    fig.text(0.5, 0.97, "Stock & Sector Breakdown — OLD YEAR", ha="center",
    ...
    fig.text(0.5, 0.97, "Trade Patterns — OLD YEAR", ha="center",
             fontsize=15, color=TEXT, fontweight="bold")
